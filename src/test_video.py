...
def evaluate_video(
    ...
    print(f"   引導模式: {guide_mode} | 單步達標門檻: {step_duration} 秒 | 決策窗口: {window_sec} 秒")
    ...
    while True:
        ...
        if left_hand is not None or right_hand is not None:
            ...
            frame_probs = classifier.predict_probabilities(features)
        else:
        ...
        if writer:
            ...
            annotated = hud.draw_hud(
                ...
                fps=fps,
                mode_str=f"Mode: {guide_mode.upper()} | Win: {window_sec}s",
            )
